...
- When **renaming a named route**, keep SPA callers working by declaring the old name as an alias: `->alias('old.name')` on the renamed route (explicit, wins over config) or batch in `config/forge.php` via `'aliases' => ['old.name' => 'new.name']`. Aliases appear as extra keys in the target's level metadata and in generated types — the frontend needs zero changes. Two usage modes: rename-migration aliases should be cleaned up once the rename settles (`php artisan route:forge:list --aliases`); long-lived stable public names for volatile routes are fine to keep forever — just update the mapping target on future renames. A dangling alias throws `AliasTargetException` (RF_BE_008).
...
- For **Blade/SSR-rendered homepages**, `@forgeSummary` optionally inlines the summary endpoint's payload into `<head>` (`window.__ROUTE_FORGE__`, a one-time self-deleting accessor) so the client skips the first summary round-trip. It embeds **only the summary** — per-tier route tables still lazy-load over HTTP — reuses the same producer/cache, adds no endpoint, and does not bump `schemeVersion`. Pure SPA / Vite dev simply omit it and fall back to the network summary.
