...
- **Local dev wiring**: `composer.json` (committed) is clean — the path repository pointing at `../route-forge-common` lives only in `composer.local.json` (git-ignored). Local install/update: `COMPOSER=composer.local.json composer update`. Keep `composer.local.json` in sync with `composer.json` when `require`/`require-dev` change. Packagist users resolve `route-forge/common` from the registry once it is tagged, so nothing to strip before publishing.
