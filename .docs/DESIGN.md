...
**别名归属的层级口径**：目标路由名被多次注册、解析到多个层级时（改名过渡期的手误形态），别名跟随**每一个**命中层级出现。理由是"别名是元信息层概念"这一决定本身：它的可见性必须与端点实际返回的 `routes` 键严格一致，否则会出现"端点里有这个键、`d.ts` 里却没有"的自相矛盾，前端按旧名取用反而被判 `UnknownRouteName`。而**计数不叠加**——`route_count` / `tier_counts` 仍按"一个别名计一次"记在 `url()` 实际解析到的末次注册层级，因为计数回答的是"目标路由住在哪"，可见性回答的是"别名能从哪儿取"，二者本就不是同一个问题；把计数也铺开会让摘要与 Laravel 自身的路由名解析语义分叉。
